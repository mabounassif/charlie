...
class MistakeClassifier:
    ...
    def __init__(self, config: Dict):
        ...
        # Validate thresholds
        if not all(key in self.thresholds for key in ['blunder', 'mistake', 'inaccuracy', 'ok']):
            raise ValueError("Missing required mistake thresholds in config")
    
    def classify_move(self, eval_before: Optional[float], eval_after: Optional[float]) -> str:
        """
    ...
    def classify_moves_batch(self, moves_data: List[Dict]) -> List[Dict]:
        """
    ...
    def get_mistake_stats(self, moves_data: List[Dict]) -> Dict:
        """
    ...
    def get_mistake_by_opening(self, moves_data: List[Dict]) -> Dict:
        """
